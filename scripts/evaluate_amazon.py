...
Two sizes, one question. At 25,612 items the closed-form item-item model still
fits in memory and can be measured beside the graph. At 162,035 items its Gram
matrix alone is 196 GiB and the inverse needs a second one, so the graph is
not the better choice, it is the only one that runs.
...
from recsys.large import (  # noqa: E402
    ease_memory, popularity_ranked, rerank, retrieve, score_ranked,
    sequential_topk,
)
...
def parse():
    ...
    parser.add_argument("--with-ease", action="store_true",
                        help="Also fit the closed-form model, if it can fit.")
    parser.add_argument("--memory-budget-gib", type=float, default=150.0)
...
def train_both(interactions, args, device, label):
    ...
    model = train_bert4rec(
        None, interactions.n_items, epochs=args.bert_epochs,
        max_length=args.max_length, batch_size=args.bert_batch,
        n_negatives=args.negatives or None,
        dataset=WindowDataset(store, args.max_length),
...
def main():
    ...
    memory = ease_memory(n_items)
    print(json.dumps(interactions.summary(), indent=2))
    print(f"\nDevice: {device}")
    print("Closed-form item-item model at this catalogue: "
          f"{memory['gram_gib']:.1f} GiB for the Gram matrix, "
          f"{memory['needed_gib']:.1f} GiB with the inverse.")
    ...
    # --------------------------------------------------------------- EASE
    ease_note = (f"not attempted: {memory['needed_gib']:.0f} GiB needed, "
                 f"budget {args.memory_budget_gib:.0f} GiB")
    if args.with_ease:
        if memory["needed_gib"] > args.memory_budget_gib:
            print(f"\nEASE skipped. {ease_note}.")
        else:
            print(f"\nEASE: allocating {memory['gram_gib']:.1f} GiB ...")
            from recsys.models import RecencyEASE

            moment = time.time()
            ease = RecencyEASE(interactions)
            elapsed = time.time() - moment
            record("ease", [ease(user, history, args.k)
                            for user, history in zip(users, histories)])
            ease_note = f"fitted in {elapsed:.0f}s"
            print(f"  EASE fitted in {elapsed:.0f}s")
